...
public class AdminChatController {
	...
	@GetMapping("/chat")
	public String adminChat(@RequestParam(value = "roomIdx", required = false) Long roomIdx,
			@AuthenticationPrincipal CustomUserDetails userDetails, HttpSession session, Model model) {
	...
	@ResponseBody
	public Map<String, Object> saveTheme(@RequestParam String theme, HttpSession session) {
	...
	@ResponseBody
	public Map<String, Object> getOrCreateDM(@RequestParam("targetUserIdx") Long targetUserIdx,
	...
	@ResponseBody
	public Map<String, Object> createChannel(@RequestParam("roomName") String roomName,
	...
	public Map<String, Object> removeMember(@PathVariable("roomIdx") Long roomIdx,
		...
		try {
			...
			if (creatorIdx != null && !creatorIdx.equals(userDetails.getUserIdx())) {
				result.put("success", false);
				result.put("msg", "방장만 멤버를 강퇴할 수 있습니다.");
				return result;
			}
	...
	@ResponseBody
	public Map<String, Object> leaveChannel(@PathVariable("roomIdx") Long roomIdx,
			@AuthenticationPrincipal CustomUserDetails userDetails) {
		Map<String, Object> result = new HashMap<>();
		...
		try {
			...
			if (creatorIdx != null && creatorIdx.equals(myIdx)) {
				result.put("success", false);
				result.put("msg", "방장은 채널을 나갈 수 없습니다. 채널 설정에서 채널을 삭제해 주세요.");
				return result;
			}
		} catch (Exception ignored) {
		}
	...
	@ResponseBody
	public Map<String, Object> toggleMute(@PathVariable("roomIdx") Long roomIdx,
			@AuthenticationPrincipal CustomUserDetails userDetails) {
		Map<String, Object> result = new HashMap<>();
		try {
	...
	@ResponseBody
	public Map<String, Object> renameChannel(@PathVariable("roomIdx") Long roomIdx,
			@RequestParam("roomName") String roomName, @AuthenticationPrincipal CustomUserDetails userDetails) {
		Map<String, Object> result = new HashMap<>();
		if (userDetails.getUserLevel() < 99) {
			result.put("success", false);
			return result;
	...
	@ResponseBody
	public Map<String, Object> transferOwnership(@PathVariable("roomIdx") Long roomIdx,
	...
	@ResponseBody
	public Map<String, Object> leaveDM(@PathVariable("roomIdx") Long roomIdx,
			@AuthenticationPrincipal CustomUserDetails userDetails) {
		Map<String, Object> result = new HashMap<>();
		try {
			Long myIdx = userDetails.getUserIdx();
	...
	@ResponseBody
	public Map<String, Object> getChatUnread(@AuthenticationPrincipal CustomUserDetails userDetails) {
		Map<String, Object> result = new HashMap<>();
		Long myIdx = userDetails != null ? userDetails.getUserIdx() : null;
		int count = 0;
		if (myIdx != null) {
			try {
				count = chatService.getUnreadTotalCount(myIdx);
			} catch (Exception ignored) {
			}
		}
	...
	@PostMapping(value = "/chat/read/{roomIdx}", produces = "application/json")
	@ResponseBody
	public Map<String, Object> markAsRead(@PathVariable("roomIdx") Long roomIdx,
			@AuthenticationPrincipal CustomUserDetails userDetails) {
		Map<String, Object> result = new HashMap<>();
		try {
			chatService.updateLastReadDate(roomIdx, userDetails.getUserIdx());
			result.put("success", true);
		} catch (Exception e) {
			result.put("success", false);
		}
