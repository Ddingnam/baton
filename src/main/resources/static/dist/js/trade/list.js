...
createApp({
    setup() {
        ...
        const write = useTradeWrite({ categories, viewMode });
        const article = useTradeArticle({});
        ...
        function goWrite() { 
			...
			Vue.nextTick(() => write.initWrite()); 
		}
        function goArticle(productIdx) { 
			viewMode.value = 'ARTICLE'; 
			Vue.nextTick(() => article.loadArticle(productIdx)); 
		}
        function goUpdate(productIdx) { 
			viewMode.value = 'WRITE';   
			Vue.nextTick(() => write.initWrite(productIdx)); 
		}
        ...
        const activeChips = computed(() => {
            ...
            if (categoryIdx.value) chips.push({ label: categories.value.find(c => String(c.CATEGORYIDX) === categoryIdx.value)?.CATEGORYNAME || '카테고리', key: 'categoryIdx' });
            if (priceMin.value || priceMax.value) chips.push({ label: (priceMin.value||'0') + '원 ~ ' + (priceMax.value||'∞') + '원', keys: ['priceMin','priceMax'] });
        ...
        async function fetchList(page = 1, append = false) {
            ...
            try {
                const res  = await fetch(ContextPath + '/api/trade/list?' + buildQs(page));
        ...
        onMounted(() => {
            ...
            document.addEventListener('keydown', e => {
                if (!article.lightboxOpen.value) return;
                if (e.key === 'Escape') article.lightboxOpen.value = false;
                if (e.key === 'ArrowLeft') article.lightboxPrev();
                if (e.key === 'ArrowRight') article.lightboxNext();
            });
        });

        return {
            viewMode, goList, goWrite, goArticle, goUpdate,
            ...
            loadMore, toggleWish, formatTimeAgo, formatPrice, getTradePlace,
            ...write,
            ...article,
            ContextPath
        };
