...
<main class="daangn-layout">
    ...
    <div class="content-container">
        ...
        <section class="profile-section"
                 onclick="location.href='${pageContext.request.contextPath}/member/profile?userIdx=${employerUserIdx}'">
                ...
                <div class="cond-item pay-highlight">
                    ...
                    <div class="cond-text">
                        ...
                        <span class="value navy-text">
                            ${alba.wageType} <fmt:formatNumber value="${alba.wage}" pattern="#,###"/>원
                        </span>
                    </div>
                </div>
        ...
        <sec:authorize access="isAuthenticated()">
            ...
            <c:if test="${loggedInUserId == alba.userIdx}">
                <section class="owner-manage-section">
                    ...
                    <div class="manage-grid">
                        ...
                        <button type="button" class="btn-manage" onclick="PullUpModule.execute(${alba.albaIdx})">
                            <i class="ri-arrow-up-circle-line"></i> 끌어올리기
                        </button>
                        <button type="button" class="btn-manage"
                                onclick="location.href='${pageContext.request.contextPath}/alba/update?postingIdx=${alba.albaIdx}'">
                            <i class="ri-edit-line"></i> 수정
                        </button>
                        <button type="button" class="btn-manage danger" onclick="confirmDelete(${alba.albaIdx})">
    ...
    <div class="bottom-inner">
        ...
        <div class="bottom-right">
            ...
            <sec:authorize access="isAnonymous()">
                <button class="btn-action btn-call" onclick="location.href='tel:${alba.tel}'">
                ...
                <c:choose>
                    <c:when test="${loggedInUserId == alba.userIdx}">
                        <button class="btn-action btn-apply full-width"
                                onclick="window.open('${pageContext.request.contextPath}/chat/albaList?albaIdx=${alba.albaIdx}', 'chatList', 'width=450,height=850')">
                    ...
                    <c:otherwise>
                        ...
                        <button class="btn-action btn-apply"
                                onclick="window.open('${pageContext.request.contextPath}/chat/room?albaIdx=${alba.albaIdx}&toUserIdx=${alba.userIdx}', 'chatRoom', 'width=450,height=850')">
                            온라인 지원
                        </button>
                    </c:otherwise>
                </c:choose>
...
<div id="statusModal" class="modal-overlay" onclick="StatusModule.close()">
    <div class="modal-content" onclick="event.stopPropagation()">
        ...
        <div class="status-options">
            <button type="button"
                    class="status-opt ${alba.recruitStatus == '모집중'   ? 'active' : ''}"
                    onclick="StatusModule.update('${alba.albaIdx}', '모집중')">모집중</button>
            <button type="button"
                    class="status-opt ${alba.recruitStatus == '모집완료' ? 'active' : ''}"
                    onclick="StatusModule.update('${alba.albaIdx}', '모집완료')">모집완료</button>
        </div>
    </div>
</div>
...
<%-- JS에서 CONTEXT_PATH 참조용 --%>
<div id="articleData"
     data-alba-idx="${alba.albaIdx}"
