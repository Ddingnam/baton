...
<body>
		...
		<div class="write-main">
			<form name="communityForm" method="post"
				...
				<input type="hidden" id="address" name="address"
					value="${dto.address}"> <input type="hidden" id="latitude"
					name="latitude" value="${dto.latitude}"> <input
					type="hidden" id="longitude" name="longitude"
					value="${dto.longitude}"> <input type="hidden"
					id="placeName" name="placeName" value="${dto.placeName}">
				...
				<div class="editor-body">
					<div class="category-pills">
						<label class="cat-pill"><input type="radio"
							name="category" value="1"
							${mode=='write' || dto.category == 1 ? 'checked' : ''}><span>일상</span></label>
						<label class="cat-pill"><input type="radio"
							name="category" value="2" ${dto.category == 2 ? 'checked' : ''}><span>동네질문</span></label>
						<label class="cat-pill"><input type="radio"
							name="category" value="3" ${dto.category == 3 ? 'checked' : ''}><span>동네맛집</span></label>
						<label class="cat-pill"><input type="radio"
							name="category" value="4" ${dto.category == 4 ? 'checked' : ''}><span>동네소식</span></label>
						<label class="cat-pill"><input type="radio"
							name="category" value="5" ${dto.category == 5 ? 'checked' : ''}><span>분실/실종</span></label>
					</div>
					...
					<div class="tag-group">
						<div class="tag-input-wrapper">
							<span class="hash-symbol">#</span> <input type="text"
								id="tagInput" class="input-tag" placeholder="태그 입력 (스페이스바 및 엔터)"
								autocomplete="off">
						</div>
					...
					<div class="poll-wrapper">
						<div class="poll-toggle-header">
							...
							<label class="switch"> <input type="checkbox"
								id="chkPollToggle" ${not empty dto.pollTitle ? 'checked' : ''}>
						...
						<div id="pollForm" class="poll-card">
							<div class="poll-input-group">
								<input type="text" name="pollTitle" id="pollTitle"
									class="input-poll-title" placeholder="무엇을 투표해볼까요?"
									autocomplete="off" value="${dto.pollTitle}">
							</div>
							<div class="poll-options-list" id="pollOptionContainer">
								<c:choose>
									<c:when test="${mode=='update' && not empty pollOptions}">
										<c:forEach var="opt" items="${pollOptions}" varStatus="status">
											<div class="poll-option-item">
												<input type="text" name="pollOptions" value="${opt}"
													class="input-option" autocomplete="off">
												<c:if test="${status.index > 1}">
													<button type="button" class="btn-del-option"
									...
									<c:otherwise>
										<div class="poll-option-item">
											<input type="text" name="pollOptions" placeholder="항목 1"
												class="input-option" autocomplete="off">
										</div>
										<div class="poll-option-item">
											<input type="text" name="pollOptions" placeholder="항목 2"
												class="input-option" autocomplete="off">
										</div>
									</c:otherwise>
								</c:choose>
							...
							<button type="button" class="btn-add-option-dashed"
							...
							<div class="poll-settings-bar">
								<div class="setting-group">
									<div class="date-picker-box">
										<i class="ri-calendar-event-line"></i> <input type="date"
											name="pollEndDate" id="pollEndDate" class="input-date-hidden"
											value="${dto.pollEndDate}"> <span id="dateDisplay">종료일
											선택</span>
									</div>
								</div>

								<div class="setting-toggles">
									<label class="mini-check" title="복수 선택 허용"> <input
										type="checkbox" name="pollMultiple" id="pollMultiple"
										${dto.pollMultiple ? 'checked' : ''}> <span
										class="check-btn">복수선택</span>
									</label> <label class="mini-check" title="익명 투표"> <input
										type="checkbox" name="pollAnonymous" id="pollAnonymous"
										${dto.pollAnonymous ? 'checked' : ''}> <span
										class="check-btn">익명</span>
									</label>
								</div>
							</div>
						</div>
					...
					<div class="location-card" id="locationCard"
						...
						<button type="button" class="btn-del-loc"
		...
		<div class="write-sidebar">
			<div class="sidebar-box">
				...
				<ul class="tip-list">
					...
					<li>판매/홍보 목적의 글은 <strong>중고거래</strong> 게시판을 이용해주세요.
					</li>
				</ul>
			</div>
		</div>
	...
	<div id="placeSearchModal" class="place-modal-overlay"
		style="display: none;">
