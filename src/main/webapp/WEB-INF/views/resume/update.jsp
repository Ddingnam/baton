...
<main class="resume-write-container">
  ...
  <form action="${pageContext.request.contextPath}/resume/update" method="post" id="resumeForm">
    ...
    <section class="form-section">
      ...
      <textarea name="introduce" id="introduceTextarea" class="form-textarea" rows="7">${dto.introduce}</textarea>
    </section>
  ...
  <div class="ai-modal" id="aiModal">
    ...
    <div class="ai-step" id="aiStep2" style="display:none;">
      ...
      <div class="ai-modal-footer" id="aiResultFooter" style="display:none;">
        <button type="button" class="btn-ai-back" id="btnAiBack"><i class="ri-arrow-left-line"></i> 다시 선택</button>
        <button type="button" class="btn-ai-apply" id="btnApply"><i class="ri-check-line"></i> 이 내용으로 적용하기</button>
      </div>
    </div>
  </div>
</div>
...
.ai-modal-backdrop{display:none;position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:2000;align-items:center;justify-content:center;}
.ai-modal-backdrop.open{display:flex;}
.ai-modal{background:#fff;border-radius:16px;width:min(640px,95vw);height:min(700px,88vh);display:flex;flex-direction:column;overflow:hidden;box-shadow:0 20px 60px rgba(0,0,0,.25);}
.ai-modal-header{padding:22px 28px 0;position:relative;flex-shrink:0;}
...
.ai-modal-body{flex:1;overflow-y:auto;padding:0 28px 16px;min-height:0;}
...
.ai-modal-footer{padding:16px 28px;border-top:1px solid #F3F4F6;display:flex;align-items:center;justify-content:space-between;flex-shrink:0;}
...
document.getElementById('btnAiOpen').addEventListener('click', () => backdrop.classList.add('open'));
